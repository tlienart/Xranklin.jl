...
html_code_block(b::Block, _)  =
    "<pre><code class=\"plaintext\">$(b |> content |> _strip |> escape_xml)</code></pre>"
latex_code_block(b::Block, _) =
    "\\begin{lstlisting}\n$(b |> content |> _strip)\\end{lstlisting}"

_lang(b) = lstrip(b.open.ss, '`')

html_code_block_lang(b::Block, _) =
    "<pre><code class=\"$(_lang(b))\">$(b |> content |> strip |> escape_xml)</code></pre>"
